...
images: []
# Not an architecture diagram: the deployment topology is covered by the NDA
# and is deliberately absent from this repo. This is the public marketing site,
# which is the part of the product anyone can already see.
thumbnail: "/images/projects/fitter-health-live.png"
hardPart: "SNS fanned out to the notification consumers with no durable buffer, so a failed delivery vanished without raising anything. It surfaced as a product complaint, not an alert."
...
{/* No architecture diagram on this page. The deployment diagram is covered by
    the NDA, so it is not published here and the image has been removed from the
    repository. The prose above stays deliberately at the level of which
    services are used and why, with no network topology, addressing or
    implementation detail. Keep it there. */}
