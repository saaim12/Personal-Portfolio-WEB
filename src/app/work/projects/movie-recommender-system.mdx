...
stack:
  ...
  - "pytest"
images: []
thumbnail: "/images/projects/movie-recommender-system-architecture.png"
hardPart: "Swapping a retrained model without dropping traffic. Hot reload inside the running service, version skew and reload safety handled there rather than by a deploy."
...
<ArchitectureDiagram
  src="/images/projects/movie-recommender-system-architecture.png"
  alt="The four MovieLens sources are merged by build_catalog and trained into a blend of content similarity, item co-occurrence and a popularity prior, on a schedule driven by an Airflow DAG. Training writes a versioned model artifact. The FastAPI serving layer, behind API-key auth with four routes and a Redis or in-memory cache in front of it, hot-reloads that artifact without a deploy."
  caption="Training on top, serving underneath, and the versioned artifact as the only thing that crosses between them."
  width={2400}
  height={1350}
/>
